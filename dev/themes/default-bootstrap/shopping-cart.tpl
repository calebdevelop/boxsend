...
{else}
					...
					{if $priceDisplay}
						<tr class="cart_total_price">
							<td rowspan="{$rowspan_total}" colspan="1" id="cart_voucher" class="cart_voucher">
								{if $voucherAllowed}
									<form class="form-inline" action="{if $opc}{$link->getPageLink('order-opc', true)}{else}{$link->getPageLink('order', true)}{/if}" method="post" id="voucher">
										<fieldset>
											<h4>{l s='Vouchers'}</h4>
											...
											<button type="submit" name="submitAddDiscount" class="button btn btn-default btnbouton btnbouton block-center"><span>{l s='OK'}</span></button>
										</fieldset>
									</form>
									{if $displayVouchers}
							...
							{if $voucherAllowed}
								<form  class="form-inline" action="{if $opc}{$link->getPageLink('order-opc', true)}{else}{$link->getPageLink('order', true)}{/if}" method="post" id="voucher">
									<fieldset>
										...
										<button type="submit" name="submitAddDiscount" class="button btn btn-default btnbouton btnbouton block-center">
											<span>{l s='OK'}</span>
										</button>
									</fieldset>
								</form>
	...
	{if $shipments && $rates && $is_single_cart}

		<form method="post" id="form-se-engine-address" action="{$link->getPageLink('order', true, NULL, 'step=1')|escape:'html':'UTF-8'}">
			<div id="idb-se-engine-cart-address">
				<!-- IDB order-address-se.tpl -->
				<div class="content">
					<div class="row">
						<div class="col-sm-9">
							<!--div class="totalItem">
								<div class="rox">
									<div class="col-xs-8 text-right">
										<div class="totalItems">TOTAL À PAYER :</div>
									</div>
									<div class="col-me-4">
										<div class="text-right">
											<div class="price product-price">4,84€ HT</div>
							...
							<div class="box-contact">
								<div class="row">
									<div class="col-xs-6 formBloc">
										<h2>Contact chez <strong>l'expéditeur</strong></h2>
										...
										<div class="bg-top-box">
											<div class="form-horizontal">
												<div class="control-group">
													<label class="radio">
														<input type="radio" name="se_address_departure[gender]" {($addressDeparture->gender == "m") ? "checked" :""} value="m" checked>M
													</label>
													<label class="radio">
														<input type="radio" name="se_address_departure[gender]" {($addressDeparture->gender == "mme") ? "checked" :""} value="mme">Mme
													</label>
												</div>

												<div class="clearfix"></div>
												<div class="control-group">
													<input type="text" name="se_address_departure[firstname]" placeholder="Nom" required value="{$addressDeparture->firstname}">
												</div>
												<div class="control-group">
													<input type="text" placeholder="Prénom" name="se_address_departure[lastname]" required value="{$addressDeparture->lastname}">
												</div>
												<div class="control-group">
													<input type="text" placeholder="Société / Organisation" name="se_address_departure[company]" value="{$shipments->from_address->company}">
												</div>
												<div class="control-group">
													<input type="text" placeholder="Adresse" value="{$shipments->from_address->street1}" name="se_address_departure[address1]" required>
												</div>
												<div class="control-group">
													<input type="text" placeholder="Complément" name="se_address_departure[address2]" {if $shipments->from_address->street2}disabled{/if} value="{$shipments->from_address->street2}">
												</div>

												<!--div class="control-group">
													<input type="text" placeholder="Suite">
												</div-->
												<div class="control-group selectBlock">
													<select disabled class="form-control" required>
														<option selected="selected" value="{$shipments->from_address->country}">{Tools::getCountryNameByIso($shipments->from_address->country,$id_lang)}</option>
													</select>
												</div>
												<div class="control-group">
													<input type="text" value="{$shipments->from_address->city}" value="{$shipments->from_address->city}" placeholder="Lyon" disabled="disabled" required>
												</div>
												<div class="control-group">
													<input type="text" value="{$shipments->from_address->zip}" value="{$shipments->from_address->city}" disabled="disabled" required>
												</div>
												<div class="control-group">
													<input type="tel" value="{$shipments->from_address->phone}" placeholder="Téléphone*">
												</div>
												<div class="control-group">
													<input type="tel" placeholder="Téléphone mobile" name="se_address_departure[phone_mobile]" value="{$addressDeparture->phone_mobile}">
												</div>
												<div class="control-group">
													<input type="mail" required placeholder="Email" name="se_address_departure[email]" value="{$addressDeparture->email}">
												</div>
												<!--div class="control-group">
													<input type="mail" placeholder="Confirmer l'email">
												</div-->
												<!--div class="control-group">
													<input type="number" placeholder="Digicode">
												</div-->
												<div class="control-group">
													<textarea placeholder="Autres informations importantes : étage, nom sur l'interphone, description des lieux etc." name="se_address_departure[other]">{$addressDeparture->other}</textarea>
												</div>
											</div>
										</div>
									</div>
									...
									<div class="col-xs-6 formBloc">
										<h2>Contact chez <strong>le destinataire</strong></h2>
										<div class="bg-top-box">
											<div class="form-horizontal">
												<div class="control-group">
													<label class="radio">
														<input type="radio" name="se_address_arrival[gender]" value="m" checked {($addressArrival->gender == "m") ? "checked" :""}>M
													</label>
													<label class="radio">
														<input type="radio" name="se_address_arrival[gender]" value="mme" {($addressArrival->gender == "mme") ? "checked" :""}>Mme
													</label>
												</div>

												<div class="clearfix"></div>
												<div class="control-group">
													<input type="text" name="se_address_arrival[firstname]" placeholder="Nom" required value="{$addressArrival->firstname}">
												</div>
												<div class="control-group">
													<input type="text" placeholder="Prénom" name="se_address_arrival[lastname]" required value="{$addressArrival->lastname}">
												</div>
												<div class="control-group">
													<input type="text" placeholder="Société / Organisation" name="se_address_arrival[company]" value="{$shipments->to_address->company}">
												</div>
												<div class="control-group">
													<input type="text" placeholder="Adresse" value="{$shipments->to_address->street1}" name="se_address_arrival[address1]" required>
												</div>
												<div class="control-group">
													<input type="text" placeholder="Complément" name="se_address_arrival[address2]" {if $shipments->to_address->street2}disabled{/if} value="{$shipments->to_address->street2}">
												</div>
												...
												<select disabled class="form-control" required>
													<option selected="selected" value="{$shipments->to_address->country}">{Tools::getCountryNameByIso($shipments->to_address->country,$id_lang)}</option>
												</select>

												<div class="control-group">
													<input type="text" value="{$shipments->to_address->city}" value="{$shipments->to_address->city}" placeholder="Lyon" disabled="disabled" required>
												</div>
												<div class="control-group">
													<input type="text" value="{$shipments->to_address->zip}" value="{$shipments->to_address->city}" disabled="disabled" required>
												</div>
												<div class="control-group">
													<input type="tel" value="{$shipments->to_address->phone}" placeholder="Téléphone*">
												</div>
												<div class="control-group">
													<input type="tel" placeholder="Téléphone mobile" name="se_address_arrival[phone_mobile]" value="{$addressArrival->phone_mobile}">
												</div>
												<div class="control-group">
													<input type="mail" required placeholder="Email" name="se_address_arrival[email]" value="{$addressArrival->email}">
												</div>
												<!--div class="control-group">
													<input type="mail" placeholder="Confirmer l'email">
												</div-->
												...
												<div class="control-group">
													<textarea placeholder="Autres informations importantes : étage, nom sur l'interphone, description des lieux etc." name="se_address_arrival[other]">{$addressArrival->other}</textarea>
												</div>
											</div>
										</div>
									</div>
								...
								<div class="row">
									<!--div class="col-xs-12">
										<h2>Adresse de <strong>Facturation</strong></h2>
										<div class="bg-top-box">
											<div class="form-horizontal">
												<div class="control-group">
													<label class="radio">
														<input type="radio" name="optionsFacturation" value="exp" checked>Expéditeur
													</label>
													<label class="radio">
														<input type="radio" name="optionsFacturation" value="dest">Destinataire
													</label>
													<label class="radio">
														<input type="radio" name="optionsFacturation" value="autre">Autre
													</label>
												</div>
											</div>
										</div>
									...
									<div class="col-xs-12">
										<h2><strong>Nom de votre envoi</strong> - facultatif</h2>
										<div class="bg-top-box">
											<div class="form-horizontal">
												<div class="row">
													<div class="col-xs-6">Choisissez un nom ou une référence pour votre envoi</div>
													<div class="col-xs-6">
														<div class="control-group">
															<input type="text" name="shipping_name" maxlength="25"><br><small class="pull-right">25 caractères maximum</small>
														</div>
													</div>
												</div>
											</div>
									...
									<!--div class="col-xs-12">
										<h2>Choix du <strong>Relais Colis <sup>®</sup></strong> de départ</h2>
										<div class="bg-top-box">
											<div class="form-horizontal">
												<div class="row">
													<div class="col-xs-7">
														<img id="exp_staticmap" alt="Voir les autres relais à proximité" title="Voir les autres Relais Colis<sup>®</sup> à proximité" src="//maps.google.com/maps/api/staticmap?zoom=14&amp;size=470x200&amp;maptype=roadmap&amp;key=AIzaSyBLLgM8ZAs7VJPDb0u3oiADgfTvWdC4jFQ&amp;center=1 RUE CARQUILLAT %2069001%20LYON 01&amp;markers=color:blue%7C1 RUE CARQUILLAT %2069001%20LYON 01">
													</div>
													<div class="col-xs-4">
														<br><br>
														<p><strong>PETIT CASINO</strong><br>
															1 RUE CARQUILLAT<br>
															69001 LYON 01<br>
															Horaires d'ouverture</p>
														<button class="button btn btn-default fistSend text-center">Je sélectionne</button><br>
														<a href="#" class="pointer">Choisir un autre Relais Colis <sup>®</sup></a>
													</div>
												</div>
											</div>
										</div>
									...
									<div class="col-xs-12">
										<h2>Choix du <strong>Relais Colis <sup>®</sup></strong> d'arrivée</h2>
										<div class="bg-top-box">
											<div class="form-horizontal">
												<div class="row">
													<div class="col-xs-12">
														<p>Une fois le Relais Colis® d'arrivée validé, l'utilisateur ne peut plus le modifier. Le choix est définitif.<br>
															Veuillez vous assurer que le destinataire pourra bien se déplacer pour retirer l'envoi. </p>
													</div>
													<div class="col-xs-7">
														<p>
															<img id="dst_staticmap" alt="Voir les autres relais à proximité" title="Voir les autres Relais Colis<sup>®</sup> à proximité" src="//maps.google.com/maps/api/staticmap?zoom=14&amp;size=470x200&amp;maptype=roadmap&amp;key=AIzaSyBLLgM8ZAs7VJPDb0u3oiADgfTvWdC4jFQ&amp;center=43 RUE DES PETITS CHAMPS %2075001%20PARIS 01&amp;markers=color:blue%7C43 RUE DES PETITS CHAMPS %2075001%20PARIS 01">
														</p>
														<div class="control-group">
															<label class="labelItem">
																<input type="checkbox" >Je souhaite laisser le choix du point de retrait au destinataire.
															</label>
														</div>
													</div>
													<div class="col-xs-4">
														<br><br>
														<p><strong>TABAC DES PETITS CHAMPS</strong><br>
															43 RUE DES PETITS CHAMPS <br>
															75001 PARIS 01<br>
															Horaires d'ouverture</p>
														<button class="button btn btn-default fistSend text-center">Je sélectionne</button><br>
														<a href="#" class="pointer">Choisir un autre Relais Colis <sup>®</sup></a>
									...
									{*
									<div class="col-xs-12">
										<h2 class="text-alerte"><img src="/modules/shippingengine/views/img/ico-alert.png" alt="*"> <strong>INFORMATIONS IMPORTANTES</strong></h2>

										<div class="box-item-list">
											<div class="sub-titre"><img src="/modules/shippingengine/views/img/ico-box2.png" alt="*"> Informations sur le contenu</div>
											<p><strong>Nature de votre envoi :</strong> Chaussures</p>
											<p><strong>Veuillez détailler le contenu de votre envoi* :</strong></p>
											<div class="control-group">
												<input type="text" name="" value="">
											</div>
											<br>
											<p class="text-alerte"><img src="/modules/shippingengine/views/img/ico-alert-small.png" alt="*"> Décrivez exactement le contenu de votre envoi, il risque sinon d'être bloqué ou détruit par le transporteur.</p><br>
											<div class="control-group">
												<label class="labelItem">
													<input type="checkbox" >Je certifie que mon envoi ne contient aucune des marchandises interdites suivantes :
												</label>
											</div>
											<div class="row no-padding">
												<div class="col-xs-6">
													<ul>
														<li>Batteries lithium seules</li>
														<li>Aérosols (déodorant, peinture...)</li>
														<li>Extincteurs ou bouteilles de gaz</li>
													</ul>
												</div>
												<div class="col-xs-6">
													<ul>
														<li>Produits à air comprimé (produits autogonflants, bateaux pneumatiques...)</li>
														<li>Animaux et insectes vivants ou morts</li>
													</ul>
												</div>
											</div>
											<br>
											<p>Ni aucun produit portant l’un de ces pictogrammes :</p>
											<p class="text-center">
												<img class="ico-forbidden" src="/modules/shippingengine/views/img/ico-forbidden-compressed-gas.jpg" alt="*">
												<img class="ico-forbidden" src="/modules/shippingengine/views/img/ico-forbidden-environmental-hazard.jpg" alt="*">
												<img class="ico-forbidden" src="/modules/shippingengine/views/img/ico-forbidden-explosive.jpg" alt="*">
												<img class="ico-forbidden" src="/modules/shippingengine/views/img/ico-forbidden-flammable.jpg" alt="*">
												<img class="ico-forbidden" src="/modules/shippingengine/views/img/ico-forbidden-harmful.jpg" alt="*">
												<img class="ico-forbidden" src="/modules/shippingengine/views/img/ico-forbidden-health-hazard.jpg" alt="*">
												<img class="ico-forbidden" src="/modules/shippingengine/views/img/ico-forbidden-oxidizing.jpg" alt="*">
												<img class="ico-forbidden" src="/modules/shippingengine/views/img/ico-forbidden-toxic.jpg" alt="*">
											</p>
											<br>
											<div class="sub-titre"><img src="/modules/shippingengine/views/img/ico-truck2.png" alt="*"> Informations du transporteur</div>
											<p>Vérifiez bien que le poids et les dimensions de votre colis sont exactes. Assurez vous que votre colis ne contient pas de denrées périssables, des bouteilles, des liquides ou des matières dangereuses ou interdites. Dans le cas contraire votre envoi pourrait être détruit en centre de tri ou refusé lors de sa prise en charge dans un Relais Colis®.</p>
											<p>L'expéditeur s'interdit de remettre en Relais Colis® des documents originaux et uniques dont le remplacement est impossible.</p>
											<br>
											<div class="sub-titre"><img src="/modules/shippingengine/views/img/ico-box-small.png" alt="*"> Informations du transporteur</div>
											<p>Afin d'éviter tous dommages sur le contenu de votre envoi, veillez à bien emballer vos colis.</p>
											<br>
											<p>Pour retrouver tous nos conseils sur la préparation de vos expéditions, <a href="#" class="pointer">cliquez ici</a> !</p>
										</div>
									</div>
									*}
									...
									<div class="col-xs-12">
										...
										<div class="bg-top-box">
											<div class="control-group">
												<label class="labelItem">
													<input type="checkbox" name="cgv" required>En cochant cette case, je déclare avoir pris connaissance des <a class="pointer"  target="_blank" href="{$link->getCMSLink('6', 'desktop')|escape:'html'}">Conditions Générales de Vente et d'Utilisation</a> du site {$shop_name} et des <a class="pointer" href="{$link->getCMSLink('7', 'desktop')|escape:'html'}" target="_blank">Conditions Générales de Vente du transporteur Relais Colis®</a> et les accepter sans réserve pour votre envoi
												</label>
											</div>
											<br>
											<div class="control-group">
												<label class="labelItem">
													<input type="checkbox" >Je souhaite recevoir les informations commerciales des Partenaires de {$shop_name}
												</label>
											</div>
										</div>
									</div>
									...
									<div class="text-center">
										<button class="button btn btn-default btnbouton btnbouton block-center">Valider ma commande</button>
									</div>
								</div>
							</div>
						</div>
						...
						<div class="col-xs-3">
							<div class="right-box-absolute sans_assurance">
								Sans assurance Sérénité (Ad Valorem), votre colis ne sera pas assuré à sa valeur réelle en cas de perte ou de dommage.
							</div>
							<a class="info-right info-1" href="#">Tout savoir sur l'assurance Ad-valorem</a>
							<div class="right-box-absolute adresse-boite-postale">Les adresses avec Boîte Postale ne sont pas traitées par les transporteurs.</div>
							<div class="right-box-absolute telephone_bulle"> <strong>Si vous ne connaissez pas le téléphone ou e-mail du destinataire,</strong>  indiquez vos coordonnées. Vous devrez le contacter pour le prévenir de la livraison.</div>
							<div class="right-box-absolute informations_bordereaux">Nous vous conseillons <strong> d'inscrire également toutes ces informations sur les bordereaux</strong>  (hors cadre)</div>
							<div class="right-box-absolute envoi_reference_interne">Pour vous permettre d'identifier facilement cet envoi avec votre référence interne.</div>
							<a class="info-right info-2" href="#">Consulter la liste des marchandises interdites</a>
						</div>
					</div>
				...
				{literal}
					<script type="text/javascript">
						$(document).ready(function(){
							$("#form-se-engine-address").validate();
						});
					</script>
				{/literal}
				<!-- END -->
