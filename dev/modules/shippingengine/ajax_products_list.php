...
$zFieldShopImage = "id_product";
if( Tools::version_compare(_PS_VERSION_,'1.6.1','<') ){
    $zFieldShopImage = "id_image";
}
...
$sql = 'SELECT p.`id_product`, pl.`link_rewrite`, p.`reference`, pl.`name`, image_shop.`id_image` id_image, il.`legend`, p.`cache_default_attribute`
		...
		LEFT JOIN `'._DB_PREFIX_.'image_shop` image_shop
			ON (image_shop.`' . $zFieldShopImage . '` = p.`id_product` AND image_shop.cover=1 AND image_shop.id_shop='.(int)$context->shop->id.')
